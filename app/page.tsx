    ...
    <div className="flex-1 space-y-12">
      ...
      <div className="relative h-64 rounded-3xl overflow-hidden mb-12 group cursor-pointer shadow-xl">
        ...
        <div className="relative z-20 h-full flex flex-col justify-center p-10">
          <span className="bg-[var(--color-primary)] text-black text-[10px] font-black px-3 py-1 rounded-full uppercase w-fit mb-4 shadow-sm">Hot Release</span>
          ...
          <button className="bg-[var(--color-primary)] text-black px-8 py-3 rounded-full text-sm font-black flex items-center gap-2 hover:bg-white transition-all w-fit shadow-lg active:scale-95">
        ...
        <div className="flex-1 space-y-16">
          ...
          <section>
            <div className="flex items-center justify-between mb-8">
              ...
              <Link href="/trending" className="text-[10px] font-black text-zinc-400 uppercase tracking-widest hover:text-[var(--color-primary)] transition-colors">See Trending</Link>
            </div>
            ...
            <div className="space-y-4">
              {songs.slice(0, 3).map((song, i) => (
                ...
                >
                  <span className="text-3xl font-black text-zinc-100 italic w-12 text-center group-hover:text-[var(--color-primary)] transition-colors">0{i+1}</span>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                       <span className="text-[10px] font-black text-[var(--color-primary)] bg-[var(--color-primary)]/10 px-2 py-0.5 rounded italic">Trending +{Math.max(1, 15 - i*2)}%</span>
                       <span className="text-[10px] font-bold text-zinc-300 uppercase">{song.plays || 0} Plays</span>
                    </div>
                  </div>
                  <div className="w-12 h-12 rounded-full border-2 border-zinc-100 flex items-center justify-center text-zinc-200 group-hover:bg-[var(--color-primary)] group-hover:border-transparent group-hover:text-black transition-all">
              ...
              {songs.length === 0 && (
                <div className="p-8 text-center bg-zinc-50 rounded-2xl border border-dashed border-zinc-200">
                  <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">No trending songs found.</p>
                </div>
              )}
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-zinc-100">
              ...
              <Link href="/trending" className="text-[10px] flex items-center gap-1 font-black text-white hover:text-black bg-black hover:bg-[var(--color-primary)] px-4 py-2 rounded-full uppercase tracking-widest transition-colors">
              ...
              {songs.map((song, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 sm:w-64 mt-2 sm:mt-0 px-2 sm:px-0">
                    ...
                    <button className="w-8 h-8 rounded-full border border-zinc-200 flex items-center justify-center text-zinc-400 group-hover:border-black group-hover:bg-black group-hover:text-[var(--color-primary)] transition-all hover:scale-105 active:scale-95 shadow-sm">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['Kalindula', 'Zed Beats', 'Hip Hop', 'Gospel', 'Amapiano', 'Dancehall'].map(genre => (
                <Link key={genre} href="/genres" className="px-4 py-2 bg-zinc-50 border border-zinc-100 rounded-lg text-xs font-bold text-zinc-600 hover:bg-[var(--color-primary)] hover:text-black hover:border-transparent transition-all shadow-sm active:scale-95">
                  {genre}
                </Link>
              ))}
            </div>
